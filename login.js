...
$FORM.addEventListener('submit', (event) => {
  ...
  req.addEventListener('load', function() {
    switch (this.status) {
      // Status Code 200 wird zwar Server-seitig nie gesendet, er ist aber das Resultat der automatische Verfolgung des Status Code 302 (erfolgreicher Login) durch das XMLHttpRequest Objektes. Deswegen erkennen wir über den Code 200 den erfolgreichen Login und leiten den Browser weiter auf die Adresse `responseURL`, welche die Startseite enthält, auf die der 302 Header `Location` verweist.
      case 200:
      ...
      // Status Code 401 (Unauthorized) wird vom Server zurückgegeben, wenn die Zugangsdaten falsch waren. Client-seitig leeren wird den Inhalt des Passwort Feldes und fokussieren es anschließend.
      case 401:
